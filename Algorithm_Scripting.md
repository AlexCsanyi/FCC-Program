...
function palindrome(str) {
  // remove all non-alphanumberic characters including the underscore _; and return lowercase
  let stripped = JSON.stringify(str).replace( /[^A-Za-z0-9]/g , '').toLowerCase();  
  // then split the stripped word into an array, reverse it, join it to string and turn it to lowercase
  let arr = stripped.split('').reverse().join('').toLowerCase();
  // check if the 2 variables match
  if (stripped == arr) {
    return true;
  } else {
    return false
